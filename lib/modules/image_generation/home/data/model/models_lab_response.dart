...
class ImageGenerationResult {
  ...
  factory ImageGenerationResult.fromJson(Map<String, dynamic> json) => ImageGenerationResult(
    ...
    futureLinks: json["future_links"] == null ? [] : List<String>.from(json["future_links"].map((x) => x)),
    createdAt: json["created_at"] != null ? DateTime.parse(json["created_at"]) : null,
    bookmarked: json["bookmarked"] ?? false,
    model: json["model"] != null ? Model.fromJson(json["model"]) : null,
    linkedResponses: json["linked_responses"] != null
        ? List<int>.from(json["linked_responses"].map((x) => x))
        : [],
  );
...
class Meta {
  ...
  Meta({required this.h, required this.w, required this.prompt, required this.seed});
  ...
  Map<String, dynamic> toJson() => {"H": h, "W": w, "prompt": prompt, "seed": seed};

  // copy with
  Meta copyWith({int? h, int? w, String? prompt, int? seed}) {
    return Meta(h: h ?? this.h, w: w ?? this.w, prompt: prompt ?? this.prompt, seed: seed ?? this.seed);
  }
}
